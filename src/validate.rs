...
/// A hostname is suspicious when a single label mixes Latin with **any**
/// non-Latin script. Pure non-Latin scripts (e.g. a fully Cyrillic, Greek,
/// Hebrew, Arabic, or Han IDN) are fine; it is the *mixing* that signals a
/// homograph attack such as `paypаl.com` (with a Cyrillic `а`) or any other
/// script substituting a Latin look-alike.
    ...
    for label in domain.split('.') {
        ...
        for c in label.chars() {
            match script_of(c) {
                ...
                Some(Script::Cyrillic) | Some(Script::Greek) | Some(Script::Other) => {
                    has_non_latin = true
                }
                None => {}
            }
...
mod tests {
    ...
    fn detects_mixed_script() {
        ...
        // Latin mixed with non-Cyrillic/Greek (e.g. Hebrew, Arabic) is also a
        // homograph signal.
        assert!(is_suspicious_mixed_script("g\u{05e8}oogle.com")); // Hebrew Resh
        assert!(is_suspicious_mixed_script("g\u{0648}ogle.com")); // Arabic Waw
                                                                  // Pure non-Latin remains fine.
        assert!(!is_suspicious_mixed_script("\u{4e2d}\u{6587}.cn"));
    }
